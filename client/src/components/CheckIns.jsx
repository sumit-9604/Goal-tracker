...
const CheckIns = () => {
  ...
  const loadGoals = async () => {
    try {
      ...
      const initial = {};
      active.forEach(g => {
        initial[g.id] = { achievement: g.actual_value ?? '', status: 'on_track' };
      });
  ...
  const getProgressPercent = (goal, achievement) => {
    ...
    switch (goal.uom_type) {
      ...
      case 'timeline':
        return actual > 0 ? Math.min((target / actual) * 100, 100).toFixed(1) : 0;
      case 'zero':
  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        {goals.map(goal => {
          const livePercent = getProgressPercent(goal, progressInputs[goal.id]?.achievement);
          return (
            <div key={goal.id} className="card p-5">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 text-sm">
                <div className="bg-blue-50 rounded-xl p-3 text-center">
                  ...
                  <p className="font-bold text-lg text-blue-700">{goal.target_value}</p>
                ...
                <div className="bg-amber-50 rounded-xl p-3 text-center">
                  ...
                  <p className="font-bold text-lg text-amber-700">{goal.weightage}%</p>
                </div>
              </div>
              ...
              {livePercent != null && (
                <div className="mb-4">
                  ...
                  <p className="text-xs text-gray-500 mt-1">{livePercent}% of target achieved</p>
                </div>
              )}
                ...
                <div className="border-t pt-4 mt-3">
                  <div className="flex flex-col sm:flex-row gap-3 items-end">
                    <div className="flex-1">
                      <label className="block text-sm font-medium mb-1">Achievement Value</label>
                      <input
                        type="number"
                        ...
                        onChange={e => setProgressInputs(prev => ({
                          ...prev,
                          [goal.id]: { ...prev[goal.id], achievement: e.target.value }
                        }))}
                      />
                    </div>
                    <div>
              ...
              {(user.role === 'manager' || user.role === 'admin') && (
                <div className="border-t pt-4 mt-3">
                  <label className="block text-sm font-medium mb-1">Add Check-in Comment</label>
                  <div className="flex gap-2">
                    ...
                    />
                    <button onClick={() => submitFeedback(goal.id)} className="btn-primary whitespace-nowrap">
                      Send Feedback
                    </button>
                  </div>
                </div>
